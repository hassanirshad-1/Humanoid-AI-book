...
if ($Help) {
    Write-Host "Usage: ./create-new-feature.ps1 [-Json] [-ShortName <name>] -FeatureDescription <description>"
    Write-Host ""
    Write-Host "Options:"
    Write-Host "  -Json                   Output in JSON format"
    Write-Host "  -ShortName <name>       Provide a custom short name (2-4 words) for the branch"
    Write-Host "  -Number N               Specify branch number manually (overrides auto-detection)"
    Write-Host "  -FeatureDescription <desc> The feature description."
    Write-Host "  -Help                   Show this help message"
    Write-Host ""
    Write-Host "Examples:"
    Write-Host "  ./create-new-feature.ps1 -FeatureDescription 'Add user authentication system' -ShortName 'user-auth'"
    Write-Host "  ./create-new-feature.ps1 -FeatureDescription 'Implement OAuth2 integration for API'"
    exit 0
}

# Check if feature description provided
if (-not $FeatureDescription) {
    Write-Error "Usage: ./create-new-feature.ps1 [-Json] [-ShortName <name>] -FeatureDescription <description>"
    exit 1
}
